...
| Wave 1 | Credibility wins | ✅ **DONE** — audit-log proof, evidence-tier spec, signup-trigger verification, the silent-DB-write fix, System Check, and now a real end-to-end audit run against live production (2026-09-14), independently verified against the database. See `docs/FIRST-AUDIT.md`. |
...
**Governance + Data Quality depth pass (2026-09-14):** built the customer/admin split as a real, DB-enforced boundary (`docs/ACCESS-ARCHITECTURE.md`) rather than a UI convention. Data Quality: added `dq_rules` (admin-only, RLS-enforced) and `dq_runs` (persisted, every prior analysis in this app was previously lost on refresh), plus a real Analyze/Rules/History UI. AI Governance: Model Registry and Use Case Registry got real register/list UI; risk-tier changes are now trigger-blocked for non-admins at the column level, verified with a real second user (viewer role registered a model, was rejected changing its risk_tier; owner succeeded on the same row). Views with real UI: 16 of 18.
