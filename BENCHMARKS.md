...
**Verified fact that frames this whole document:** the `ai.gov-prod` Supabase project has **2 registered users** (1 email signup, 1 anonymous guest — both independently confirmed working), **0 audits ever run**, 0 rows in `fairness_metrics` or `explainability_reports`. See `docs/SYSTEM-AUDIT.md` for the full, currently-verified system inventory. The schema is real and advisor-verified. Nothing has used it. That gap — schema vs. live usage — is the single most important thing to hold in mind reading everything below.
...
| Zero live usage | Needs real users signing up and running real audits — a distribution/adoption problem, not a code problem. **De-risked substantially:** the signup trigger is verified (including slug-collision and special-character edge cases), a silent DB-write failure was found and fixed, and an in-app System Check now names the exact failure point. The top remaining suspect is a Supabase dashboard redirect setting, which cannot be fixed from the repo. **Partially addressed:** the audit-log path has now been exercised end-to-end with real writes during immutability/tamper testing, so the "never been exercised by anything" claim is no longer fully true of that subsystem | **Go-to-market**, not engineering |
